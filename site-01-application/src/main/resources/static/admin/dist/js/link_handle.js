$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [
            ...
            {label: '添加时间', name: 'addTime', index: 'addTime', width: 100}
        ],
        height: 560,
        rowNum: 10,
        rowList: [10, 20, 50],
...
$('#saveButton').click(function () {
    ...
    var params = {
        "linkName": linkName, "linkType": linkType, "linkUrl": linkUrl,
        "linkDescription": linkDescription, "linkRank": linkRank
    };
    var url = '/backstage/links/';
    if (linkId != null && linkId > 0) {
        url = '/backstage/links/'+linkId;
    }
    $.ajax({
        type: 'POST',//方法类型
        url: url,
...
function linkEdit() {
    ...
    reset();
    //请求数据
    $.get("/backstage/links/" + id, function (r) {
        if (r.code === "000" && r.data != null) {
            //填充数据至modal
